...
// Forwards admin updates to data-ingestion service. [id] is the IATA code.
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const iataCode = (params.id || '').toUpperCase()
    const body = await req.json()

    // Convert UI 0-10 theme scores to backend 0-100 if provided
    ...
    const res = await fetch(`${base}/api/v1/data/destinations/${iataCode}`, {
      method: 'PATCH',
