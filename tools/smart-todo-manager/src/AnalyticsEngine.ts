import { Todo, ProjectTodos, TodoAnalytics } from './types'
...
export class AnalyticsEngine {
  ...
  private calculateCompletionTrends(todos: Todo[]): { date: Date; completed: number; added: number }[] {
    const trends: Map<string, { completed: number; added: number }> = new Map()
  ...
  private calculateCategoryBreakdown(todos: Todo[]): { category: string; total: number; completed: number }[] {
    const categories: Map<string, { total: number; completed: number }> = new Map()
  ...
  private calculatePriorityDistribution(todos: Todo[]): { priority: string; count: number }[] {
  ...
  private calculateVelocityHistory(todos: Todo[]): { week: Date; completed: number; estimated: number }[] {
    const velocity: Map<string, { completed: number; estimated: number }> = new Map()
  ...
  private identifyBottlenecks(projectTodos: ProjectTodos): { reason: string; count: number; affectedTodos: string[] }[] {
    const bottlenecks: Map<string, string[]> = new Map()
    ...
    const stuckTodos = projectTodos.projectTodos.filter(t => 
      t.status === 'in_progress' && t.updatedAt < oneWeekAgo
    )
    ...
    const urgentPending = projectTodos.projectTodos.filter(t => 
      t.status === 'pending' && 
  ...
  private calculateTimeAccuracy(todos: Todo[]): { estimated: number; actual: number; accuracy: number }[] {
    const accuracy: { estimated: number; actual: number; accuracy: number }[] = []
    
    const completedWithEstimates = todos.filter(t => 
      t.status === 'completed' && 
      t.estimatedHours && 
      t.actualHours &&
      t.estimatedHours > 0 &&
  ...
  private identifyBlockingReason(todo: Todo, projectTodos: ProjectTodos): string {
    // Check dependencies
  ...
  private findTodoById(id: string, projectTodos: ProjectTodos): Todo | undefined {
    return [
      ...projectTodos.projectTodos,
      ...projectTodos.archivedTodos,
      ...projectTodos.futureTodos
