...
export class FutureTodoParser {
  private futureTodoPath: string

  constructor(futureTodoPath: string) {
    this.futureTodoPath = futureTodoPath
  }

  async parseFutureTodos(): Promise<Todo[]> {
    try {
  ...
  async updateFutureTodoMd(futureTodos: Todo[], projectTodos: Todo[], archivedTodos: Todo[]): Promise<void> {
    try {
  ...
  private parseMarkdownTodos(content: string): Todo[] {
    const todos: Todo[] = []
  ...
  private updateTodoStatuses(content: string, allTodos: Todo[]): string {
    const lines = content.split('\n')
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (todoMatch) {
        ...
        const matchingTodo = allTodos.find(todo => 
          this.generateIdFromContent(todoContent.trim()) === todo.id ||
  ...
  private addProgressTracking(content: string, projectTodos: Todo[], archivedTodos: Todo[]): string {
    ...
    const recentlyCompleted = archivedTodos.filter(t => 
...
`
  ...
  private extractCategory(sectionTitle: string): string {
    const title = sectionTitle.toLowerCase()
    
    if (title.includes('phase 1') || title.includes('deployment')) return 'deployment'
    if (title.includes('phase 2') || title.includes('creator') || title.includes('community')) return 'community'
    if (title.includes('phase 3') || title.includes('feature')) return 'features'
    if (title.includes('phase 4') || title.includes('business') || title.includes('analytics')) return 'business'
  ...
  private extractPriorityFromSection(sectionTitle: string): 'low' | 'medium' | 'high' | 'critical' {
    const title = sectionTitle.toLowerCase()
    
    if (title.includes('critical') || title.includes('phase 1')) return 'critical'
    if (title.includes('high') || title.includes('phase 2')) return 'high'
    if (title.includes('medium') || title.includes('phase 3')) return 'medium'
  ...
  private generateIdFromContent(content: string): string {
  ...
  private calculateAverageCompletionTime(completedTodos: Todo[]): string {
    const todosWithTimes = completedTodos.filter(t => 
      t.completedAt && t.createdAt
    )
    
    if (todosWithTimes.length === 0) return 'N/A'
  ...
  private getUpcomingTasks(projectTodos: Todo[]): Todo[] {
    return projectTodos
      .filter(t => t.status === 'pending' && t.priority === 'high')
      .slice(0, 5)
  }

  private async createEmptyFutureTodoFile(): Promise<void> {
