...
async function createTodoManager(configOverrides = {}) {
    const config = { ...getDefaultConfig(), ...configOverrides };
    const todoManager = new SmartTodoManager_1.SmartTodoManager(config);
...
program
    ...
    try {
        ...
        let sessionTodos = [];
        if (options.session) {
            const sessionData = await Promise.resolve(`${path.resolve(options.session)}`).then(s => __importStar(require(s)));
    ...
    try {
        ...
        for (let i = 0; i < Math.min(suggestions.length, parseInt(options.count)); i++) {
            ...
            if (context?.filePaths && todo.filePaths) {
                const relevantFiles = todo.filePaths.filter(file => context.filePaths.some((contextFile) => file.includes(contextFile) || contextFile.includes(file)));
...
program
    ...
    try {
        const todoManager = await createTodoManager();
        if (options.gitHooks) {
            const gitIntegration = todoManager.gitIntegration;
...
program
    ...
    try {
        const todoManager = await createTodoManager();
        const projectTodos = todoManager.projectTodos;
