...
exports.CompletionDetector = void 0;
class CompletionDetector {
    constructor(gitIntegration) {
        this.gitIntegration = gitIntegration;
    }
    async checkTodoCompletion(todo, recentCommits) {
    ...
    async checkPattern(todo, pattern, recentCommits) {
        switch (pattern.type) {
            case 'file_exists':
    ...
    async checkFileExistsPattern(todo, pattern) {
        if (!pattern.pattern) {
            return null;
        }
    ...
    async checkFileContainsPattern(todo, pattern) {
        if (!pattern.pattern) {
            return null;
        }
    ...
    async checkCommitMessagePattern(todo, pattern, recentCommits) {
    ...
    async checkBuildSuccessPattern(todo, pattern) {
    ...
    async checkTestPassPattern(todo, pattern) {
    ...
    async heuristicDetection(todo, recentCommits) {
        ...
        if (todo.filePaths && todo.filePaths.length > 0) {
            for (const commit of recentCommits) {
                const relevantFiles = commit.changedFiles.filter(file => todo.filePaths.some(todoFile => file.includes(todoFile) || todoFile.includes(file)));
    ...
    calculateCommitRelevance(todo, commit) {
        const todoWords = this.extractKeywords(todo.content);
        ...
        for (const todoWord of todoWords) {
            if (commitWords.some(commitWord => commitWord.toLowerCase().includes(todoWord.toLowerCase()) ||
        ...
        const hasCompletionWord = completionWords.some(word => commit.message.toLowerCase().includes(word));
    ...
    async detectCreationCompletion(todo, recentCommits) {
        ...
        if (content.includes('create') || content.includes('add') || content.includes('implement')) {
            ...
            if (targetName) {
                ...
                for (const commit of recentCommits) {
                    const createdFiles = commit.changedFiles.filter(file => file.toLowerCase().includes(targetName.toLowerCase()) ||
    ...
    extractCreationTarget(todoContent) {
...
exports.CompletionDetector = CompletionDetector;
//# sourceMappingURL=CompletionDetector.js.map